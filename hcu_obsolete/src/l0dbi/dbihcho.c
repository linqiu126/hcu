...
INSERT INTO `hcudb`.`hcuhchoze08ch2odatainfo` (
`sid` ,
`deviceid` ,
`timestamp` ,
`dataformat`,
...
//存储HCHO数据，每一次存储，都是新增一条记录
//由于是本地数据库，这里不考虑网格问题，直接存储，简单处理
OPSTAT dbi_HcuHchoZe08ch2oDataInfo_save(sensor_hcho_ze08ch2o_data_element_t *hchoData)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
	...
	//存入新的数据
    sprintf(strsql, "INSERT INTO `hcuhchoze08ch2odatainfo` (deviceid, timestamp, dataformat, hchovalue) VALUES \
    		('%d', '%d', '%d', '%d')", hchoData->equipid, hchoData->timeStamp, hchoData->dataFormat, hchoData->hchoValue);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIHCHO: INSERT data error: %s\n", mysql_error(sqlHandler));
...
//删除对应用户所有超过90天的数据
//缺省做成90天，如果参数错误，导致90天以内的数据强行删除，则不被认可
OPSTAT dbi_HcuHchoZe08ch2oDataInfo_delete_3monold(UINT32 days)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
    ...
    cursec = time(NULL);
    days = days * 24 * 3600;
    sprintf(strsql, "DELETE FROM `hcuhchoze08ch2odatainfo` WHERE (%d - `timestamp` > '%d')", cursec, days);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIHCHO: INSET data error: %s\n", mysql_error(sqlHandler));

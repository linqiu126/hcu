...
HcuFsmStateItem_t FsmNoise[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_IDLE,            				fsm_noise_init},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_IDLE,            				fsm_com_do_nothing},

	//Task level initialization
    {MSG_ID_COM_INIT,       	FSM_STATE_NOISE_INITED,            		fsm_noise_init},
    {MSG_ID_COM_RESTART,		FSM_STATE_NOISE_INITED,            		fsm_noise_restart},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_NOISE_INITED,            		fsm_com_do_nothing},

    //Normal working status
    {MSG_ID_COM_RESTART,        			FSM_STATE_NOISE_ACTIVED,          		fsm_noise_restart},
    {MSG_ID_COM_INIT_FEEDBACK,				FSM_STATE_NOISE_ACTIVED,            	fsm_com_do_nothing},
	{MSG_ID_COM_HEART_BEAT,       			FSM_STATE_NOISE_ACTIVED,          		fsm_com_heart_beat_rcv},
    ...
    {MSG_ID_COM_INIT_FEEDBACK,				FSM_STATE_NOISE_MODBUS_WFFB,        	fsm_com_do_nothing},
	{MSG_ID_COM_HEART_BEAT,       			FSM_STATE_NOISE_MODBUS_WFFB,        	fsm_com_heart_beat_rcv},
	{MSG_ID_COM_HEART_BEAT_FB,       		FSM_STATE_NOISE_MODBUS_WFFB,        	fsm_com_do_nothing},
	{MSG_ID_COM_TIME_OUT,       			FSM_STATE_NOISE_MODBUS_WFFB,        	fsm_noise_time_out},
    ...
    {MSG_ID_COM_RESTART,        			FSM_STATE_NOISE_SPSVIRGO_WFFB,         	fsm_noise_restart},
    {MSG_ID_COM_INIT_FEEDBACK,				FSM_STATE_NOISE_SPSVIRGO_WFFB,         	fsm_com_do_nothing},
	{MSG_ID_COM_HEART_BEAT,       			FSM_STATE_NOISE_SPSVIRGO_WFFB,         	fsm_com_heart_beat_rcv},
	{MSG_ID_COM_HEART_BEAT_FB,       		FSM_STATE_NOISE_SPSVIRGO_WFFB,         	fsm_com_do_nothing},
	{MSG_ID_COM_TIME_OUT,       			FSM_STATE_NOISE_SPSVIRGO_WFFB,         	fsm_noise_time_out},
...
gTaskNoiseContext_t zSensorNoiseInfo[MAX_NUM_OF_SENSOR_NOISE_INSTALLED];
UINT8 gTaskNoiseContext.currentSensorId;
...
OPSTAT fsm_noise_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("NOISE: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NOISE], zHcuSysCrlTab.taskRun.taskName[src_id]);
			return FAILURE;
		}
	}
	...
	memset(zSensorNoiseInfo, 0, sizeof(gTaskNoiseContext_t));
...
OPSTAT fsm_noise_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_NOISE] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NOISE]++;
			HcuErrorPrint("NOISE: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NOISE], zHcuSysCrlTab.taskRun.taskName[TASK_ID_NOISE]);
			return FAILURE;
		}
	}
...
void func_noise_time_out_processing_no_rsponse(void)
{
	...
	//恢复当前传感器的空闲状态
	zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwAccess = SENSOR_NOISE_HW_ACCESS_IDLE;
	zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwStatus = SENSOR_NOISE_HW_STATUS_DEACTIVE;
	zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].busyCount = 0;
...
void func_noise_time_out_read_data_from_modbus(void)
{
	...
	if (zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwAccess == SENSOR_NOISE_HW_ACCESS_BUSY){
		//多次等待后强行恢复
		if ((zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].busyCount < 0) || (zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].busyCount >= SENSOR_NOISE_HW_ACCESS_BUSY_COUNT_NUM_MAX)){
			zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].busyCount = 0;
			zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwAccess =SENSOR_NOISE_HW_ACCESS_IDLE;
		}else{
			zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].busyCount++;
			//Keep busy status
		}
	...
	//如果当前传感器处于空闲态，干活！
	else if (zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwAccess == SENSOR_NOISE_HW_ACCESS_IDLE){
		//Do somemthing
		...
		snd.equId = zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].equId;
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NOISE]++;
			HcuErrorPrint("NOISE: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NOISE], zHcuSysCrlTab.taskRun.taskName[TASK_ID_MODBUS]);
			return;
		}
		...
		zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwAccess = SENSOR_NOISE_HW_ACCESS_BUSY;
		zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].busyCount = 0;
	...
	//任何其他状态，强制初始化
		...
		zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwAccess = SENSOR_NOISE_HW_ACCESS_IDLE;
		zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwStatus = SENSOR_NOISE_HW_STATUS_ACTIVE;  //假设缺省为活跃状态
		zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].busyCount = 0;
	}
...
void func_noise_time_out_read_data_from_spsvirgo(void)
{
	...
	//如果当前传感器处于空闲态，干活！
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NOISE]++;
			HcuErrorPrint("NOISE: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NOISE], zHcuSysCrlTab.taskRun.taskName[TASK_ID_SPSVIRGO]);
			return;
		}
		else
		{
...
OPSTAT fsm_noise_data_report_from_modbus(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//在线模式
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NOISE]++;
			HcuErrorPrint("NOISE: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NOISE], zHcuSysCrlTab.taskRun.taskName[TASK_ID_CLOUDVELA]);
			return FAILURE;
		}
	...
	//恢复当前传感器的空闲状态
	zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwAccess = SENSOR_NOISE_HW_ACCESS_IDLE;
	zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].hwStatus = SENSOR_NOISE_HW_STATUS_ACTIVE;
	zSensorNoiseInfo[gTaskNoiseContext.currentSensorId].busyCount = 0;

...
HcuFsmStateItem_t FsmAirprs[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_IDLE,            				fsm_airprs_restart},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_IDLE,            				fsm_com_do_nothing},

	//Task level initialization
    {MSG_ID_COM_INIT,       	FSM_STATE_AIRPRS_INITIED,            	fsm_airprs_init},
    {MSG_ID_COM_RESTART,		FSM_STATE_AIRPRS_INITIED,            	fsm_airprs_restart},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_AIRPRS_INITIED,            	fsm_com_do_nothing},

    //Task level initialization
    {MSG_ID_COM_RESTART,        FSM_STATE_AIRPRS_ACTIVED,            	fsm_airprs_restart},
	...
	{MSG_ID_COM_TIME_OUT,       FSM_STATE_AIRPRS_ACTIVED,          		fsm_airprs_time_out},
...
OPSTAT fsm_airprs_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("AIRPRS: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_AIRPRS], zHcuSysCrlTab.taskRun.taskName[src_id]);
			return FAILURE;
		}
	}
...
OPSTAT fsm_airprs_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_AIRPRS] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_AIRPRS]++;
			HcuErrorPrint("AIRPRS: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_AIRPRS], zHcuSysCrlTab.taskRun.taskName[TASK_ID_AIRPRS]);
			return FAILURE;
		}
	}
...
OPSTAT func_airprs_time_out_read_data_from_bmp180(void)
{
	...
	//存入数据库
	if ((HCU_SENSOR_DATA_SAVE_TO_LOCAL_DB_SET == HCU_SYSCFG_SENSOR_SAVE_TO_LOCAL_DB_FLAG_YES) && (zHcuI2cAirprsBmp180 >= HCU_SENSOR_AIRPRS_VALUE_MIN) && (zHcuI2cAirprsBmp180 <= HCU_SENSOR_AIRPRS_VALUE_MAX))
	{
	...
	//存入数据库
	if ((HCU_SENSOR_DATA_SAVE_TO_LOCAL_DB_SET == HCU_SYSCFG_SENSOR_SAVE_TO_LOCAL_DB_FLAG_YES) && (zHcuI2cAltitudeBmp180 >= HCU_SENSOR_ALTITUDE_VALUE_MIN) && (zHcuI2cAltitudeBmp180 <= HCU_SENSOR_ALTITUDE_VALUE_MAX))
	{

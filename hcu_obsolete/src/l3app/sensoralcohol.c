...
HcuFsmStateItem_t FsmAlcohol[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_IDLE,            				fsm_alcohol_restart},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_IDLE,            				fsm_com_do_nothing},

	//Task level initialization
    {MSG_ID_COM_INIT,       	FSM_STATE_ALCOHOL_INITIED,            	fsm_alcohol_init},
    {MSG_ID_COM_RESTART,		FSM_STATE_ALCOHOL_INITIED,            	fsm_alcohol_restart},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_ALCOHOL_INITIED,            	fsm_com_do_nothing},

    //Task level initialization
    {MSG_ID_COM_RESTART,        FSM_STATE_ALCOHOL_ACTIVED,            	fsm_alcohol_restart},
	...
	{MSG_ID_COM_TIME_OUT,       FSM_STATE_ALCOHOL_ACTIVED,          		fsm_alcohol_time_out},
...
OPSTAT fsm_alcohol_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("ALCOHOL: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_ALCOHOL], zHcuSysCrlTab.taskRun.taskName[src_id]);
			return FAILURE;
		}
	}
	...
	//启动周期性定时器
	ret = hcu_timer_start(TASK_ID_ALCOHOL, TIMER_ID_1S_ALCOHOL_PERIOD_READ, zHcuSysEngPar.timer.alcoholReqTimer, TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);
	if (ret == FAILURE){
	...
	while(1){
		...
		if (dataLen > 1){
			...
			if (ret == FAILURE){
				...
				HcuErrorPrint("ALCOHOL: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_ALCOHOL], zHcuTaskNameList[TASK_ID_HSMMP]);
				return FAILURE;
			}
		}
...
OPSTAT fsm_alcohol_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_ALCOHOL] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_ALCOHOL]++;
			HcuErrorPrint("ALCOHOL: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_ALCOHOL], zHcuSysCrlTab.taskRun.taskName[TASK_ID_ALCOHOL]);
			return FAILURE;
		}
	}
...
OPSTAT func_alcohol_time_out_read_data_from_mq3alco(void)
{
	...
	//存入数据库
	if ((HCU_SENSOR_DATA_SAVE_TO_LOCAL_DB_SET == HCU_SYSCFG_SENSOR_SAVE_TO_LOCAL_DB_FLAG_YES) && (zHcuGpioAlcoholMq3alco >= HCU_SENSOR_ALCOHOL_VALUE_MIN) && (zHcuGpioAlcoholMq3alco <= HCU_SENSOR_ALCOHOL_VALUE_MAX))
	{

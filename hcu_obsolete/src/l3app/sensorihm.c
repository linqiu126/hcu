...
OPSTAT fsm_ihm_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("IHM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_IHM], zHcuSysCrlTab.taskRun.taskName[src_id]);
			return FAILURE;
		}
	}
	...
	while(1){
		...
		if (dataLen > 1){
			...
			if (ret == FAILURE){
				...
				HcuErrorPrint("IHM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_IHM], zHcuTaskNameList[TASK_ID_HSMMP]);
				return FAILURE;
			}
		}
...
OPSTAT fsm_ihm_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_IHM] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_IHM]++;
			HcuErrorPrint("IHM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_IHM], zHcuSysCrlTab.taskRun.taskName[TASK_ID_IHM]);
			return FAILURE;
		}
	}
...
OPSTAT fsm_ihm_nbiotcj188_data_req(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (ret == FAILURE){
		zHcuSysStaPm.taskRunErrCnt[TASK_ID_IHM]++;
		HcuErrorPrint("IHM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_IHM], zHcuSysCrlTab.taskRun.taskName[TASK_ID_NBIOTCJ188]);
		return FAILURE;
	}

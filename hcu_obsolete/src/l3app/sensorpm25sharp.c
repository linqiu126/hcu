...
*/
HcuFsmStateItem_t FsmPm25Sharp[] =
{
    ...
    {MSG_ID_COM_RESTART,        FSM_STATE_PM25SHARP_ACTIVED,            fsm_pm25sharp_restart},
	...
	{MSG_ID_COM_TIME_OUT,       FSM_STATE_PM25SHARP_ACTIVED,          	fsm_pm25sharp_time_out},
...
OPSTAT fsm_pm25sharp_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("PM25SHARP: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_PM25SHARP], zHcuSysCrlTab.taskRun.taskName[src_id]);
			return FAILURE;
		}
	}
...
OPSTAT fsm_pm25sharp_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_PM25SHARP] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_PM25SHARP]++;
			HcuErrorPrint("PM25SHARP: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_PM25SHARP], zHcuSysCrlTab.taskRun.taskName[TASK_ID_PM25SHARP]);
			return FAILURE;
		}
	}
		...
		if(j==6)
		{
		  ...
		  if(pm25_frame_received_buff[5]==sum && pm25_frame_received_buff[6]== 0xff ) //终止位是0xFF
		  {
			  ...
			  if((end_time - start_time) > 2)
			  {
				  ...
				  if ((HCU_SENSOR_DATA_SAVE_TO_LOCAL_DB_SET == HCU_SYSCFG_SENSOR_SAVE_TO_LOCAL_DB_FLAG_YES) && (average_pm25 >= HCU_SENSOR_PM25_VALUE_MIN) && (average_pm25 <= HCU_SENSOR_PM25_VALUE_MAX))
				  {

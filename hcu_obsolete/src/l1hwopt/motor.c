...
HcuFsmStateItem_t FsmMotor[] =
{
    ...
    {MSG_ID_COM_RESTART,        FSM_STATE_MOTOR_ACTIVIED,            		fsm_motor_restart},
...
OPSTAT fsm_motor_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("MOTOR: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_MOTOR], zHcuSysCrlTab.taskRun.taskName[src_id]);
			return FAILURE;
		}
	}

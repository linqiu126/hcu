...
OPSTAT fsm_nbiotqg376_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("NBIOTQG376: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo.taskName[TASK_ID_NBIOTQG376], zHcuTaskInfo.taskName[src_id]);
			return FAILURE;
		}
	}
...
OPSTAT fsm_nbiotqg376_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuRunErrCnt[TASK_ID_NBIOTQG376] > HCU_RUN_ERROR_LEVEL_3_CRITICAL){
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_NBIOTQG376]++;
			HcuErrorPrint("NBIOTQG376: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo.taskName[TASK_ID_NBIOTQG376], zHcuTaskInfo.taskName[TASK_ID_NBIOTQG376]);
			return FAILURE;
		}
	}

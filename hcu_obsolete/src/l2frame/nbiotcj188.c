...
*/
HcuFsmStateItem_t FsmNbiotcj188[] =
{
	...
	{MSG_ID_IPM_NBIOTCJ188_CONTROL_FB,   	FSM_STATE_NBIOTCJ188_ONLINE, 	fsm_nbiotcj188_ipm_ctrl_resp},
...
OPSTAT fsm_nbiotcj188_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("NBIOTCJ188: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NBIOTCJ188], zHcuSysCrlTab.taskRun.taskName[src_id]);
			return FAILURE;
		}
	}
...
OPSTAT fsm_nbiotcj188_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_NBIOTCJ188] > HCU_RUN_ERROR_LEVEL_3_CRITICAL){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NBIOTCJ188]++;
			HcuErrorPrint("NBIOTCJ188: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NBIOTCJ188], zHcuSysCrlTab.taskRun.taskName[TASK_ID_NBIOTCJ188]);
			return FAILURE;
		}
	}
...
OPSTAT func_nbiotcj188_dl_msg_unpack(CloudDataSendBuf_t *buf)
{
	...
	if ((equType >= HCU_NBIOT_CJ188_T_TYPE_WATER_METER_MIN) && (equType <= HCU_NBIOT_CJ188_T_TYPE_WATER_METER_MAX)){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NBIOTCJ188]++;
			HcuErrorPrint("NBIOTCJ188: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NBIOTCJ188], zHcuSysCrlTab.taskRun.taskName[TASK_ID_IWM]);
			return FAILURE;
		}
	}//IWM
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NBIOTCJ188]++;
			HcuErrorPrint("NBIOTCJ188: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NBIOTCJ188], zHcuSysCrlTab.taskRun.taskName[TASK_ID_IHM]);
			return FAILURE;
		}
	}//IHM
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NBIOTCJ188]++;
			HcuErrorPrint("NBIOTCJ188: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NBIOTCJ188], zHcuSysCrlTab.taskRun.taskName[TASK_ID_IGM]);
			return FAILURE;
		}
	}//IGM
	else if ((equType >= HCU_NBIOT_CJ188_T_TYPE_GAS_METER_MIN) && (equType <= HCU_NBIOT_CJ188_T_TYPE_GAS_METER_MAX)){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_NBIOTCJ188]++;
			HcuErrorPrint("NBIOTCJ188: Send message error, TASK [%s] to TASK[%s]!\n", zHcuSysCrlTab.taskRun.taskName[TASK_ID_NBIOTCJ188], zHcuSysCrlTab.taskRun.taskName[TASK_ID_IPM]);
			return FAILURE;
		}
	}//IPM
	else{

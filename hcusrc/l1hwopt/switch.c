...
** FSM of the SWITCH
*/
HcuFsmStateItem_t HcuFsmSwitch[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_COMMON,            			fsm_switch_restart},
...
OPSTAT fsm_switch_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_SWITCH, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_switch_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("SWITCH: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_SWITCH].taskName, zHcuVmCtrTab.task[src_id].taskName);
			return FAILURE;
		}
	}

	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_SWITCH, FSM_STATE_SWITCH_INITED);
	...
	//Global Variables
	zHcuSysStaPm.taskRunErrCnt[TASK_ID_SWITCH] = 0;

	//设置状态机到目标状态
	FsmSetState(TASK_ID_SWITCH, FSM_STATE_SWITCH_RECEIVED);
	HCU_DEBUG_PRINT_FAT("SWITCH: Enter FSM_STATE_SWITCH_ACTIVED status, Keeping refresh here!\n");
	/*
	...
	while(1){
		...
		if (dataLen > 1){
			...
			if (ret == FAILURE){
				...
				HcuErrorPrint("SWITCH: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_SWITCH].taskName, zHcuTaskInfo[TASK_ID_HSMMP].taskName);
				return FAILURE;
			}
		}

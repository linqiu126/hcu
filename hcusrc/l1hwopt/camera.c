...
HcuFsmStateItem_t HcuFsmCamera[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_COMMON,            			fsm_camera_restart},
...
OPSTAT fsm_camera_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_CAMERA, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_camera_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_CAMERA, FSM_STATE_CAMERA_INITED);
	...
	//设置状态机到目标状态
	FsmSetState(TASK_ID_CAMERA, FSM_STATE_CAMERA_ACTIVIED);
	HCU_DEBUG_PRINT_FAT("CAMERA: Enter FSM_STATE_CAMERA_ACTIVED status, Keeping refresh here!\n");

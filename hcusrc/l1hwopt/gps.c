...
HcuFsmStateItem_t HcuFsmGps[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_COMMON,            			fsm_gps_restart},
...
//Global variables
//msgie_struct_gps_pos_info_t zHcuVmCtrTab.hwinv.gps;
...
OPSTAT fsm_gps_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_GPS, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_gps_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_GPS, FSM_STATE_GPS_INITED);
	...
	*/
	...
	//设置状态机到目标状态
	FsmSetState(TASK_ID_GPS, FSM_STATE_GPS_RECEIVED);
	HCU_DEBUG_PRINT_FAT("GPS: Enter FSM_STATE_GPS_ACTIVED status, Keeping refresh here!\n");

...
** FSM of the SPS485
*/
HcuFsmStateItem_t HcuFsmSps485[] =
{
...
OPSTAT fsm_sps485_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_SPS485, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_sps485_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("SPS485: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_SPS485].taskName, zHcuVmCtrTab.task[src_id].taskName);
			return FAILURE;
		}
	}

	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_SPS485, FSM_STATE_SPS485_INITED);
	...
	//设置状态机到目标状态
	FsmSetState(TASK_ID_SPS485, FSM_STATE_SPS485_RECEIVED);
	HCU_DEBUG_PRINT_FAT("SPS485: Enter FSM_STATE_SPS485_ACTIVED status, Keeping refresh here!\n");

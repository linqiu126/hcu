...
OPSTAT fsm_gpio_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_GPIO, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_gpio_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_GPIO, FSM_STATE_GPIO_INITED);
	...
	zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOTEMPDHT11].fVal = HCU_SENSOR_VALUE_NULL;
	zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOHUMIDDHT11].fVal = HCU_SENSOR_VALUE_NULL;
	zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOTOXICGASMQ135].fVal = HCU_SENSOR_VALUE_NULL;
	zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOALCOHOLMQ3ALCO].fVal = HCU_SENSOR_VALUE_NULL;
	zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOTOXICGASZP01VOC].fVal = HCU_SENSOR_VALUE_NULL;

	//设置状态机到目标状态
	FsmSetState(TASK_ID_GPIO, FSM_STATE_GPIO_ACTIVIED);
	HCU_DEBUG_PRINT_FAT("GPIO: Enter FSM_STATE_GPIO_ACTIVED status, Keeping refresh here!\n");
/*
	int workingCycle = 4;
	//进入循环工作模式
	while(1){

...
HcuFsmStateItem_t HcuFsmPwm[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_COMMON,            			fsm_pwm_restart},
...
OPSTAT fsm_pwm_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_PWM, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_pwm_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("PWM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_PWM].taskName, zHcuVmCtrTab.task[src_id].taskName);
			return FAILURE;
		}
	}

	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_PWM, FSM_STATE_PWM_INITED);
	...
	//设置状态机到目标状态
	FsmSetState(TASK_ID_PWM, FSM_STATE_PWM_ACTIVIED);
	HCU_DEBUG_PRINT_FAT("PWM: Enter FSM_STATE_PWM_ACTIVED status, Keeping refresh here!\n");

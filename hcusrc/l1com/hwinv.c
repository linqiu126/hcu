...
OPSTAT func_hwinv_global_par_init(void)
{
	...
	strcat(stmp, "/");
	strcat(stmp,zHcuSysEngPar.hwBurnId.equLable);
	strcat(stmp,"_");
	...
	//特殊的干净文件名字
	strcpy(zHcuVmCtrTab.clock.curAvorionFnameH264,zHcuSysEngPar.hwBurnId.equLable);
...
OPSTAT hcu_hwinv_engpar_read_pop_data_into_mem(void)
{
	...
	if ((HCU_SYSCFG_HW_MASSIVE_PRODUTION_SET == HCU_SYSCFG_HW_MASSIVE_PRODUTION_NO) && (HCU_SYSCFG_INIT_SET_BY_VM_STATIC_TABLE_GENERAL_SET == HCU_SYSCFG_INIT_SET_BY_VM_STATIC_TABLE_YES)){
		...
		strncpy(zHcuSysEngPar.swm.hcuSwRunDir, HCU_SYSCFG_SWM_LOCAL_RUN_DIR_DEFAULT, (sizeof(HCU_SYSCFG_SWM_LOCAL_RUN_DIR_DEFAULT)<sizeof(zHcuSysEngPar.swm.hcuSwRunDir))?(sizeof(HCU_SYSCFG_SWM_LOCAL_RUN_DIR_DEFAULT)):(sizeof(zHcuSysEngPar.swm.hcuSwRunDir)));

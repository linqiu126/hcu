...
OPSTAT fsm_l3aqycg10_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuRunErrCnt[TASK_ID_L3AQYCG10] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_L3AQYCG10]++;
			HcuErrorPrint("L3AQYCG10: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_L3AQYCG10], zHcuTaskNameList[TASK_ID_L3AQYCG10]);
			return FAILURE;
		}
	}

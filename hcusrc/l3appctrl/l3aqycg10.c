...
HcuFsmStateItem_t HcuFsmL3aqycg10[] =
{
	...
	{MSG_ID_ENTRY,       						FSM_STATE_ENTRY,            			fsm_l3aqycg10_task_entry}, //Starting

	//System level initialization, only controlled by HCU-MAIN
    {MSG_ID_COM_INIT,       					FSM_STATE_IDLE,            				fsm_l3aqycg10_init},
    {MSG_ID_COM_RESTART,						FSM_STATE_IDLE,            				fsm_l3aqycg10_restart},
    ...
    {MSG_ID_COM_INIT_FEEDBACK,					FSM_STATE_L3AQYCG10_ACTIVED,            		fsm_com_do_nothing},
	{MSG_ID_COM_HEART_BEAT,     				FSM_STATE_L3AQYCG10_ACTIVED,       			fsm_com_heart_beat_rcv},
	{MSG_ID_COM_HEART_BEAT_FB,  				FSM_STATE_L3AQYCG10_ACTIVED,       			fsm_com_do_nothing},
	{MSG_ID_COM_TIME_OUT,       				FSM_STATE_L3AQYCG10_ACTIVED,          			fsm_l3aqycg10_time_out},
...
OPSTAT fsm_l3aqycg10_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	//除了对全局变量进行操作之外，尽量不要做其它操作，因为该函数将被主任务/线程调用，不是本任务/线程调用
	//该API就是给本任务一个提早介入的入口，可以帮着做些测试性操作
	if (FsmSetState(TASK_ID_L3AQYCG10, FSM_STATE_IDLE) == FAILURE){
		HcuErrorPrint("L3AQYCG10: Error Set FSM State at fsm_l3aqycg10_task_entry\n");}
	return SUCCESS;
}

OPSTAT fsm_l3aqycg10_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	int ret=0;
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("L3AQYCG10: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_L3AQYCG10].taskName, zHcuVmCtrTab.task[src_id].taskName);
			return FAILURE;
		}
	}

	//收到初始化消息后，进入初始化状态
	if (FsmSetState(TASK_ID_L3AQYCG10, FSM_STATE_L3AQYCG10_INITED) == FAILURE){
		HcuErrorPrint("L3AQYCG10: Error Set FSM State!\n");
		return FAILURE;
	}

	//初始化硬件接口
	if (func_l3aqycg10_int_init() == FAILURE){
		HcuErrorPrint("L3AQYCG10: Error initialize interface!\n");
		return FAILURE;
	}
	...
	//启动周期性定时器
	ret = hcu_timer_start(TASK_ID_L3AQYCG10, TIMER_ID_1S_L3AQYCG10_PERIOD_READ, \
			zHcuSysEngPar.timer.array[TIMER_ID_1S_L3AQYCG10_PERIOD_READ].dur, TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);
	if (ret == FAILURE){
	...
	if ((zHcuSysEngPar.debugMode & HCU_TRACE_DEBUG_FAT_ON) != FALSE){
		HcuDebugPrint("L3AQYCG10: Enter FSM_STATE_L3AQYCG10_ACTIVED status, Keeping refresh here!\n");
	}
...
OPSTAT fsm_l3aqycg10_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	int ret;
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_L3AQYCG10] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_L3AQYCG10]++;
			HcuErrorPrint("L3AQYCG10: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_L3AQYCG10].taskName, zHcuVmCtrTab.task[TASK_ID_L3AQYCG10].taskName);
			return FAILURE;
		}
	}

	//Period time out received
	if ((rcv.timeId == TIMER_ID_1S_L3AQYCG10_PERIOD_READ) &&(rcv.timeRes == TIMER_RESOLUTION_1S)){

...
FsmStateItem_t FsmL3opwldtdr[] =
{
    ...
    {MSG_ID_COM_INIT,       					FSM_STATE_L3OPWLOTDR_INITIED,            		fsm_l3opwldtdr_init},
    {MSG_ID_COM_RESTART,						FSM_STATE_L3OPWLOTDR_INITIED,            		fsm_l3opwldtdr_restart},
    {MSG_ID_COM_INIT_FEEDBACK,					FSM_STATE_L3OPWLOTDR_INITIED,            		fsm_com_do_nothing},
...
OPSTAT fsm_l3opwldtdr_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("L3OPWLOTDR: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_L3OPWLOTDR], zHcuTaskNameList[src_id]);
			return FAILURE;
		}
	}

	//收到初始化消息后，进入初始化状态
	if (FsmSetState(TASK_ID_L3OPWLOTDR, FSM_STATE_L3OPWLOTDR_INITED) == FAILURE){
		HcuErrorPrint("L3OPWLOTDR: Error Set FSM State!\n");
		return FAILURE;
	}
	...
	//Global Variables
	zHcuRunErrCnt[TASK_ID_L3OPWLOTDR] = 0;

	//启动周期性定时器
	ret = hcu_timer_start(TASK_ID_L3OPWLOTDR, TIMER_ID_1S_L3OPWLOTDR_PERIOD_READ, HCU_L3OPWLOTDR_TIMER_DURATION_PERIOD_READ, TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);
	if (ret == FAILURE){
		zHcuRunErrCnt[TASK_ID_L3OPWLOTDR]++;
		HcuErrorPrint("L3OPWLOTDR: Error start period timer!\n");
		return FAILURE;
	}

	//设置状态机到目标状态
	if (FsmSetState(TASK_ID_L3OPWLOTDR, FSM_STATE_L3OPWLOTDR_ACTIVED) == FAILURE){
		zHcuRunErrCnt[TASK_ID_L3OPWLOTDR]++;
		HcuErrorPrint("L3OPWLOTDR: Error Set FSM State!\n");
		return FAILURE;
	}
	if ((zHcuSysEngPar.debugMode & HCU_TRACE_DEBUG_FAT_ON) != FALSE){
		HcuDebugPrint("L3OPWLOTDR: Enter FSM_STATE_L3OPWLOTDR_ACTIVED status, Keeping refresh here!\n");
	}
...
OPSTAT fsm_l3opwldtdr_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((param_ptr == NULL || param_len > sizeof(msg_struct_com_time_out_t))){
		HcuErrorPrint("L3OPWLOTDR: Receive message error!\n");
		zHcuRunErrCnt[TASK_ID_L3OPWLOTDR]++;
		return FAILURE;
	}
	memcpy(&rcv, param_ptr, param_len);

	//钩子在此处，检查zHcuRunErrCnt[TASK_ID_L3OPWLOTDR]是否超限
	if (zHcuRunErrCnt[TASK_ID_L3OPWLOTDR] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		//减少重复RESTART的概率
		zHcuRunErrCnt[TASK_ID_L3OPWLOTDR] = zHcuRunErrCnt[TASK_ID_L3OPWLOTDR] - HCU_RUN_ERROR_LEVEL_2_MAJOR;
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_L3OPWLOTDR]++;
			HcuErrorPrint("L3OPWLOTDR: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_L3OPWLOTDR], zHcuTaskNameList[TASK_ID_L3OPWLOTDR]);
			return FAILURE;
		}
	}

	//Period time out received
	if ((rcv.timeId == TIMER_ID_1S_L3OPWLOTDR_PERIOD_READ) &&(rcv.timeRes == TIMER_RESOLUTION_1S)){
		//保护周期读数的优先级，强制抢占状态，并简化问题
		if (FsmGetState(TASK_ID_L3OPWLOTDR) != FSM_STATE_L3OPWLOTDR_ACTIVED){
			ret = FsmSetState(TASK_ID_L3OPWLOTDR, FSM_STATE_L3OPWLOTDR_ACTIVED);
			if (ret == FAILURE){
				zHcuRunErrCnt[TASK_ID_L3OPWLOTDR]++;
				HcuErrorPrint("L3OPWLOTDR: Error Set FSM State!\n");
				return FAILURE;

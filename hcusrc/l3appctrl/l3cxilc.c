...
HcuFsmStateItem_t HcuFsmL3cxilc[] =
{
	...
	{MSG_ID_ENTRY,       						FSM_STATE_ENTRY,            			fsm_l3cxilc_task_entry}, //Starting
...
OPSTAT fsm_l3cxilc_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	//除了对全局变量进行操作之外，尽量不要做其它操作，因为该函数将被主任务/线程调用，不是本任务/线程调用
	//该API就是给本任务一个提早介入的入口，可以帮着做些测试性操作
	if (FsmSetState(TASK_ID_L3CXILC, FSM_STATE_IDLE) == FAILURE){
		HcuErrorPrint("L3CXILC: Error Set FSM State at fsm_l3cxilc_task_entry\n");}
	return SUCCESS;
}

OPSTAT fsm_l3cxilc_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	int ret=0;
	...
	//收到初始化消息后，进入初始化状态
	if (FsmSetState(TASK_ID_L3CXILC, FSM_STATE_L3CXILC_INITED) == FAILURE){
		HcuErrorPrint("L3CXILC: Error Set FSM State!\n");
		return FAILURE;
	}

	//初始化硬件接口
	if (func_l3cxilc_int_init() == FAILURE){
		HcuErrorPrint("L3CXILC: Error initialize interface!\n");
		return FAILURE;
	}
	...
	//启动周期性定时器
	ret = hcu_timer_start(TASK_ID_L3CXILC, TIMER_ID_1S_L3CXILC_PERIOD_READ, \
			zHcuSysEngPar.timer.array[TIMER_ID_1S_L3CXILC_PERIOD_READ].dur, TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);
	if (ret == FAILURE){
	...
	if ((zHcuSysEngPar.debugMode & HCU_TRACE_DEBUG_FAT_ON) != FALSE){
		HcuDebugPrint("L3CXILC: Enter FSM_STATE_L3CXILC_ACTIVED status, Keeping refresh here!\n");
	}
...
OPSTAT fsm_l3cxilc_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	int ret;
	...
	if ((rcv.timeId == TIMER_ID_1S_L3CXILC_PERIOD_READ) &&(rcv.timeRes == TIMER_RESOLUTION_1S)){

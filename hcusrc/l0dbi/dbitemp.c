...
INSERT INTO `hcudb`.`hcutempdatainfo` (
`sid` ,
`deviceid` ,
`timestamp` ,
`dataformat`,
...
INSERT INTO `hcudb`.`hcutempbmp180datainfo` (
`sid` ,
`deviceid` ,
`timestamp` ,
`dataformat`,
...
INSERT INTO `hcudb`.`hcutempmth01datainfo` (
`sid` ,
`deviceid` ,
`timestamp` ,
`dataformat`,
...
OPSTAT dbi_HcuTempDataInfo_save(sensor_temp_data_element_t *tempData)
{
	MYSQL *sqlHandler;
    int result = 0;
...
//查询满足条件的第一条记录
//deviceid是入参，指针为出参
OPSTAT dbi_HcuTempDataInfo_inqury_1st_record(UINT32 deviceid, sensor_temp_data_element_t *tempData)
{
	MYSQL *sqlHandler;
	MYSQL_RES *resPtr;
	MYSQL_ROW sqlRow;
	...
	if (!resPtr){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBITEMP: mysql_use_result error!\n");
        return FAILURE;
	}
	...
	if ((sqlRow = mysql_fetch_row(resPtr)) == NULL)
	{
    	...
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBITEMP: mysql_fetch_row NULL error!\n");
        return FAILURE;
	}
	else{
		UINT32 index = 2;
		if (sqlRow[index]) tempData->timeStamp = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) tempData->dataFormat = (UINT8)(atol(sqlRow[index++]) & 0xFF);
		if (sqlRow[index]) tempData->tempValue = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) tempData->gps.ew = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) tempData->gps.gpsx = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) tempData->gps.ns = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) tempData->gps.gpsy = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) tempData->gps.gpsz = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) tempData->onOffLineFlag = (UINT8)(atol(sqlRow[index++]) & 0xFF);
	}
...
OPSTAT dbi_HcuTempDataInfo_delete_3monold(UINT32 days)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
...
//存储TEMP数据，每一次存储，都是新增一条记录
//由于是本地数据库，这里不考虑网格问题，直接存储，简单处理
OPSTAT dbi_HcuTempBmp180DataInfo_save(sensor_temp_bmp180_data_element_t *tempData)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
...
//删除对应用户所有超过90天的数据
//缺省做成90天，如果参数错误，导致90天以内的数据强行删除，则不被认可
OPSTAT dbi_HcuTempBmp180DataInfo_delete_3monold(UINT32 days)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
    ...
    cursec = time(NULL);
    days = days * 24 * 3600;
    sprintf(strsql, "DELETE FROM `hcutempbmp180datainfo` WHERE (%d - `timestamp` > '%d')", cursec, days);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBITEMP: INSET data error: %s\n", mysql_error(sqlHandler));
...
//存储TEMP数据，每一次存储，都是新增一条记录
//由于是本地数据库，这里不考虑网格问题，直接存储，简单处理
OPSTAT dbi_HcuTempMth01DataInfo_save(sensor_temp_mth01_data_element_t *tempData)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
...
//删除对应用户所有超过90天的数据
//缺省做成90天，如果参数错误，导致90天以内的数据强行删除，则不被认可
OPSTAT dbi_HcuTempMth01DataInfo_delete_3monold(UINT32 days)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
    ...
    cursec = time(NULL);
    days = days * 24 * 3600;
    sprintf(strsql, "DELETE FROM `hcutempmth01datainfo` WHERE (%d - `timestamp` > '%d')", cursec, days);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBITEMP: INSET data error: %s\n", mysql_error(sqlHandler));
...
OPSTAT dbi_HcuTempDataInfo_GetMin(UINT32 dur, HcuSysMsgIeL3aqycContextStaElement_t *tempData)
{
	MYSQL *sqlHandler;
    int result = 0;
	MYSQL_RES *resPtr;
    ...
    sprintf(strsql, "SELECT MIN(tempvalue) FROM `hcutempdatainfo` WHERE (%d - `timestamp` < '%d')", cursec, duration);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBITEMP: Get Minimum data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}
...
OPSTAT dbi_HcuTempDataInfo_GetMax(UINT32 dur, HcuSysMsgIeL3aqycContextStaElement_t *tempData)
{
	MYSQL *sqlHandler;
    int result = 0;
	MYSQL_RES *resPtr;
    ...
    //Get the minimum data
    sprintf(strsql, "SELECT MAX(tempvalue) FROM `hcutempdatainfo` WHERE (%d - `timestamp` < '%d')", cursec, duration);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBITEMP: Get Maximum data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}
...
OPSTAT dbi_HcuTempDataInfo_GetAvg(UINT32 dur, HcuSysMsgIeL3aqycContextStaElement_t *tempData)
{
	MYSQL *sqlHandler;
    int result = 0;
	MYSQL_RES *resPtr;
    ...
    //Get the minimum data
    sprintf(strsql, "SELECT AVG(tempvalue) FROM `hcutempdatainfo` WHERE (%d - `timestamp` < '%d')", cursec, duration);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBITEMP: Get Avg data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}

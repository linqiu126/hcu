/*
 * dbiwindspd.c
 *
 *  Created on: 2016年1月5日
 *      Author: hitpony
 */

#include "../l0dbi/dbiwindspd.h"
#include "../l0service/trace.h"
#include "../l0dbi/dbicom.h"

//WINDSPD数据表格式
// HcuWindspdDataInfo
/*
CREATE TABLE IF NOT EXISTS `hcuwindspddatainfo` (
  `sid` int(4) NOT NULL AUTO_INCREMENT,
  `deviceid` int(4) NOT NULL,
  `timestamp` int(4) NOT NULL,
  `dataformat` int(1) NOT NULL,
  `windspdvalue` int(4) NOT NULL,
  `ew` char(1) NOT NULL,
  `gpsx` int(4) NOT NULL,
  `ns` char(1) NOT NULL,
  `gpsy` int(4) NOT NULL,
  `gpsz` int(4) NOT NULL,
  `onofflineflag` int(2) NOT NULL,
  PRIMARY KEY (`sid`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8;

INSERT INTO `hcudb`.`hcuwindspddatainfo` (
`sid` ,
`deviceid` ,
`timestamp` ,
`dataformat`,
`windspdvalue` ,
`ew` ,
`gpsx` ,
`ns` ,
`gpsy` ,
`gpsz` ,
`onofflineflag`
)
VALUES (
'1', '3', '2', '1', '4', 'e', '5', 'n', '6', '7', '8'
);

*/

//存储WINDSPD数据，每一次存储，都是新增一条记录
//由于是本地数据库，这里不考虑网格问题，直接存储，简单处理
OPSTAT dbi_HcuWindspdDataInfo_save(sensor_windspd_data_element_t *windspdData)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];

    //入参检查：不涉及到生死问题，参数也没啥大问题，故而不需要检查，都可以存入数据库表单中
    if (windspdData == NULL){
    	HcuErrorPrint("DBIWINDSPD: Input parameter NULL pointer!\n");
        return FAILURE;
    }

	//建立连接
//    sqlHandler = mysql_init(NULL);
//    if(!sqlHandler)
//    {
//    	HcuErrorPrint("DBIWINDSPD: MySQL init failed!\n");
//        return FAILURE;
//    }
//    sqlHandler = mysql_real_connect(sqlHandler, zHcuSysEngPar.dbi.hcuDbHost, zHcuSysEngPar.dbi.hcuDbUser, zHcuSysEngPar.dbi.hcuDbPsw, zHcuSysEngPar.dbi.hcuDbName, zHcuSysEngPar.dbi.hcuDbPort, NULL, 0);  //unix_socket and clientflag not used.
//    if (!sqlHandler){
//    	mysql_close(sqlHandler);
//    	HcuErrorPrint("DBIWINDSPD: MySQL connection failed: %s\n", mysql_error(sqlHandler));
//        return FAILURE;
//    }
    HCU_L0DBICOM_INIT_DB_CONN();

    HcuDebugPrint("hcuwindspddatainfo: deviceid = %d, timestamp = %d, dataformat = %d, windspdvalue = %d, ew = %c, gpsx = %d, ns = %c, gpsy = %d, gpsz = %d, onofflineflag = %d\n\n\n", windspdData->equipid, windspdData->timeStamp, windspdData->dataFormat, windspdData->windspdValue, windspdData->gps.ew, windspdData->gps.gpsx, windspdData->gps.ns, windspdData->gps.gpsy, windspdData->gps.gpsz, windspdData->onOffLineFlag);

	//存入新的数据
    sprintf(strsql, "INSERT INTO `hcuwindspddatainfo` (deviceid, timestamp, dataformat, windspdvalue, ew, gpsx, ns, gpsy, gpsz, onofflineflag) VALUES \
    		('%d', '%d', '%d', '%d', '%c', '%d', '%c', '%d', '%d', '%d')", windspdData->equipid, windspdData->timeStamp, windspdData->dataFormat, windspdData->windspdValue, windspdData->gps.ew, windspdData->gps.gpsx, windspdData->gps.ns, windspdData->gps.gpsy, windspdData->gps.gpsz, windspdData->onOffLineFlag);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: INSERT data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}

	//释放记录集
    mysql_close(sqlHandler);
	if ((zHcuSysEngPar.debugMode & HCU_SYSCFG_TRACE_DEBUG_NOR_ON) != FALSE){
		HcuDebugPrint("DBIWINDSPD: WINDSPD data record save to DB!\n");
	}
    return SUCCESS;
}

//查询满足条件的第一条记录
//deviceid是入参，指针为出参
OPSTAT dbi_HcuWindspdDataInfo_inqury_1st_record(UINT32 deviceid, sensor_windspd_data_element_t *windspdData)
{
	MYSQL *sqlHandler;
	MYSQL_RES *resPtr;
	MYSQL_ROW sqlRow;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];

    //入参检查：不涉及到生死问题，参数也没啥大问题，故而不需要检查，都可以存入数据库表单中
    if (windspdData == NULL){
    	HcuErrorPrint("DBIWINDSPD: Input parameter NULL pointer!\n");
        return FAILURE;
    }

	//建立数据库连接
//    sqlHandler = mysql_init(NULL);
//    if(!sqlHandler)
//    {
//    	HcuErrorPrint("DBIWINDSPD: MySQL init failed!\n");
//        return FAILURE;
//    }
//    sqlHandler = mysql_real_connect(sqlHandler, zHcuSysEngPar.dbi.hcuDbHost, zHcuSysEngPar.dbi.hcuDbUser, zHcuSysEngPar.dbi.hcuDbPsw, zHcuSysEngPar.dbi.hcuDbName, zHcuSysEngPar.dbi.hcuDbPort, NULL, 0);  //unix_socket and clientflag not used.
//    if (!sqlHandler){
//    	mysql_close(sqlHandler);
//    	HcuErrorPrint("DBIWINDSPD: MySQL connection failed!\n");
//        return FAILURE;
//    }
    HCU_L0DBICOM_INIT_DB_CONN();

	//获取数据
    sprintf(strsql, "SELECT * FROM `hcuwindspddatainfo` WHERE (`deviceid` = '%d')", deviceid);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: INSET data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}

	//查具体的结果
	resPtr = mysql_use_result(sqlHandler);
	if (!resPtr){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: mysql_use_result error!\n");
        return FAILURE;
	}

	//只读取第一条记录
	if ((sqlRow = mysql_fetch_row(resPtr)) == NULL)
	{
		mysql_free_result(resPtr);
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: mysql_fetch_row NULL error!\n");
        return FAILURE;
	}
	else{
		UINT32 index = 2;
		if (sqlRow[index]) windspdData->timeStamp = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) windspdData->dataFormat = (UINT8)(atol(sqlRow[index++]) & 0xFF);
		if (sqlRow[index]) windspdData->windspdValue = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) windspdData->gps.ew = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) windspdData->gps.gpsx = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) windspdData->gps.ns = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) windspdData->gps.gpsy = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) windspdData->gps.gpsz = (UINT32)atol(sqlRow[index++]);
		if (sqlRow[index]) windspdData->onOffLineFlag = (UINT8)(atol(sqlRow[index++]) & 0xFF);
	}

	//释放记录集
	mysql_free_result(resPtr);
    mysql_close(sqlHandler);
    return SUCCESS;
}

//删除对应用户所有超过90天的数据
//缺省做成90天，如果参数错误，导致90天以内的数据强行删除，则不被认可
OPSTAT dbi_HcuWindspdDataInfo_delete_3monold(UINT32 days)
{
	MYSQL *sqlHandler;
    int result = 0;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
    UINT32 cursec = 0;

    //入参检查：不涉及到生死问题，参数也没啥大问题，故而不需要检查，都可以存入数据库表单中
    if (days <WINDSPD_DATA_SAVE_DAYS_MIN) days = WINDSPD_DATA_SAVE_DAYS_MIN;

	//建立连接
//    sqlHandler = mysql_init(NULL);
//    if(!sqlHandler)
//    {
//    	HcuErrorPrint("DBIWINDSPD: MySQL init failed!\n");
//        return FAILURE;
//    }
//    sqlHandler = mysql_real_connect(sqlHandler, zHcuSysEngPar.dbi.hcuDbHost, zHcuSysEngPar.dbi.hcuDbUser, zHcuSysEngPar.dbi.hcuDbPsw, zHcuSysEngPar.dbi.hcuDbName, zHcuSysEngPar.dbi.hcuDbPort, NULL, 0);  //unix_socket and clientflag not used.
//    if (!sqlHandler){
//    	mysql_close(sqlHandler);
//    	HcuErrorPrint("DBIWINDSPD: MySQL connection failed!\n");
//        return FAILURE;
//    }
    HCU_L0DBICOM_INIT_DB_CONN();

	//删除满足条件的数据
    cursec = time(NULL);
    days = days * 24 * 3600;
    sprintf(strsql, "DELETE FROM `hcuwindspddatainfo` WHERE (%d - `timestamp` > '%d')", cursec, days);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: INSET data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}

	//释放记录集
    mysql_close(sqlHandler);
    return SUCCESS;
}


//Get Min Max Avg accoriding time duration by shanchun
OPSTAT dbi_HcuWindspdDataInfo_GetMin(UINT32 dur, HcuSysMsgIeL3aqycContextStaElement_t *windspdData)
{
	MYSQL *sqlHandler;
    int result = 0;
	MYSQL_RES *resPtr;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
	MYSQL_ROW sqlRow;
    UINT32 cursec = 0;
    UINT32 duration =0;

	UINT32 index = 0;
	//UINT32 min;

	//建立连接
//    sqlHandler = mysql_init(NULL);
//    if(!sqlHandler)
//    {
//    	HcuErrorPrint("DBIWINDSPD: MySQL init failed!\n");
//        return FAILURE;
//    }
//    sqlHandler = mysql_real_connect(sqlHandler, zHcuSysEngPar.dbi.hcuDbHost, zHcuSysEngPar.dbi.hcuDbUser, zHcuSysEngPar.dbi.hcuDbPsw, zHcuSysEngPar.dbi.hcuDbName, zHcuSysEngPar.dbi.hcuDbPort, NULL, 0);  //unix_socket and clientflag not used.
//    if (!sqlHandler){
//    	mysql_close(sqlHandler);
//    	HcuErrorPrint("DBIWINDSPD: MySQL connection failed!\n");
//        return FAILURE;
//    }
    HCU_L0DBICOM_INIT_DB_CONN();

    cursec = time(NULL);
    duration = dur;

    //Get the minimum data
    sprintf(strsql, "SELECT MIN(windspdvalue) FROM `hcuwindspddatainfo` WHERE (%d - `timestamp` < '%d')", cursec, duration);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: Get Minimum data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}

	resPtr = mysql_use_result(sqlHandler);
	if (!resPtr){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: mysql_use_result error!\n");
        return FAILURE;
	}


	if ((sqlRow = mysql_fetch_row(resPtr)) == NULL)
	{
		mysql_free_result(resPtr);
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: mysql_fetch_row NULL error!\n");
        return FAILURE;
	}
	else{

		if (sqlRow[index]) windspdData->a01007_Min = ((UINT32)atol(sqlRow[index]))*DB_SENSOR_DATA_FOMAT_FLOAT_WITH_NF1;
		//HCU_DEBUG_PRINT_INF("DBIWINDSPD: min=%4.3f\n", windspdData->a01007_Min);
	}

	//释放记录集
	mysql_free_result(resPtr);
    mysql_close(sqlHandler);
    return SUCCESS;
}

OPSTAT dbi_HcuWindspdDataInfo_GetMax(UINT32 dur, HcuSysMsgIeL3aqycContextStaElement_t *windspdData)
{
	MYSQL *sqlHandler;
    int result = 0;
	MYSQL_RES *resPtr;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
	MYSQL_ROW sqlRow;
    UINT32 cursec = 0;
    UINT32 duration =0;

	UINT32 index = 0;
	//UINT32 max;

	//建立连接
//    sqlHandler = mysql_init(NULL);
//    if(!sqlHandler)
//    {
//    	HcuErrorPrint("DBIWINDSPD: MySQL init failed!\n");
//        return FAILURE;
//    }
//    sqlHandler = mysql_real_connect(sqlHandler, zHcuSysEngPar.dbi.hcuDbHost, zHcuSysEngPar.dbi.hcuDbUser, zHcuSysEngPar.dbi.hcuDbPsw, zHcuSysEngPar.dbi.hcuDbName, zHcuSysEngPar.dbi.hcuDbPort, NULL, 0);  //unix_socket and clientflag not used.
//    if (!sqlHandler){
//    	mysql_close(sqlHandler);
//    	HcuErrorPrint("DBIWINDSPD: MySQL connection failed!\n");
//        return FAILURE;
//    }
    HCU_L0DBICOM_INIT_DB_CONN();

    cursec = time(NULL);
    duration = dur;

    //Get the minimum data
    sprintf(strsql, "SELECT MAX(windspdvalue) FROM `hcuwindspddatainfo` WHERE (%d - `timestamp` < '%d')", cursec, duration);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: Get Maximum data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}

	resPtr = mysql_use_result(sqlHandler);
	if (!resPtr){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: mysql_use_result error!\n");
        return FAILURE;
	}


	if ((sqlRow = mysql_fetch_row(resPtr)) == NULL)
	{
		mysql_free_result(resPtr);
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: mysql_fetch_row NULL error!\n");
        return FAILURE;
	}
	else{

		if (sqlRow[index]) windspdData->a01007_Max = ((UINT32)atol(sqlRow[index]))*DB_SENSOR_DATA_FOMAT_FLOAT_WITH_NF1;
		//HCU_DEBUG_PRINT_INF("DBIWINDSPD: max=%4.3f\n", windspdData->a01007_Max);
	}

	//释放记录集
	mysql_free_result(resPtr);
    mysql_close(sqlHandler);
    return SUCCESS;
}

OPSTAT dbi_HcuWindspdDataInfo_GetAvg(UINT32 dur, HcuSysMsgIeL3aqycContextStaElement_t *windspdData)
{
	MYSQL *sqlHandler;
    int result = 0;
	MYSQL_RES *resPtr;
    char strsql[DBI_MAX_SQL_INQUERY_STRING_LENGTH];
	MYSQL_ROW sqlRow;
    UINT32 cursec = 0;
    UINT32 duration =0;

	UINT32 index = 0;
	//UINT32 avg;

	//建立连接
//    sqlHandler = mysql_init(NULL);
//    if(!sqlHandler)
//    {
//    	HcuErrorPrint("DBIWINDSPD: MySQL init failed!\n");
//        return FAILURE;
//    }
//    sqlHandler = mysql_real_connect(sqlHandler, zHcuSysEngPar.dbi.hcuDbHost, zHcuSysEngPar.dbi.hcuDbUser, zHcuSysEngPar.dbi.hcuDbPsw, zHcuSysEngPar.dbi.hcuDbName, zHcuSysEngPar.dbi.hcuDbPort, NULL, 0);  //unix_socket and clientflag not used.
//    if (!sqlHandler){
//    	mysql_close(sqlHandler);
//    	HcuErrorPrint("DBIWINDSPD: MySQL connection failed!\n");
//        return FAILURE;
//    }
    HCU_L0DBICOM_INIT_DB_CONN();

    cursec = time(NULL);
    duration = dur;

    //Get the minimum data
    sprintf(strsql, "SELECT AVG(windspdvalue) FROM `hcuwindspddatainfo` WHERE (%d - `timestamp` < '%d')", cursec, duration);
	result = mysql_query(sqlHandler, strsql);
	if(result){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: Get Avg data error: %s\n", mysql_error(sqlHandler));
        return FAILURE;
	}

	resPtr = mysql_use_result(sqlHandler);
	if (!resPtr){
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: mysql_use_result error!\n");
        return FAILURE;
	}


	if ((sqlRow = mysql_fetch_row(resPtr)) == NULL)
	{
		mysql_free_result(resPtr);
    	mysql_close(sqlHandler);
    	HcuErrorPrint("DBIWINDSPD: mysql_fetch_row NULL error!\n");
        return FAILURE;
	}
	else{

		if (sqlRow[index]) windspdData->a01007_Avg = ((UINT32)atol(sqlRow[index]))*DB_SENSOR_DATA_FOMAT_FLOAT_WITH_NF1;
		//HCU_DEBUG_PRINT_INF("DBIWINDSPD: avg=%4.3f\n", windspdData->a01007_Avg);
	}

	//释放记录集
	mysql_free_result(resPtr);
    mysql_close(sqlHandler);
    return SUCCESS;
}

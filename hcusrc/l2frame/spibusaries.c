...
*/
HcuFsmStateItem_t HcuFsmSpibusaries[] =
{
...
OPSTAT fsm_spibusaries_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_SPIBUSARIES, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_spibusaries_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("SPIBUSARIES: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_SPIBUSARIES].taskName, zHcuVmCtrTab.task[src_id].taskName);
			return FAILURE;
		}
	}

	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_SPIBUSARIES, FSM_STATE_SPIBUSARIES_INITED);
	...
	//设置状态机到目标状态
	FsmSetState(TASK_ID_SPIBUSARIES, FSM_STATE_SPIBUSARIES_ACTIVED);
	HCU_DEBUG_PRINT_FAT("SPIBUSARIES: Enter FSM_STATE_SPIBUSARIES_ACTIVED status, Keeping refresh here!\n");
	/*

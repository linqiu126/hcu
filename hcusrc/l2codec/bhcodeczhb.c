...
OPSTAT func_cloudvela_stdzhb_msg_heart_beat_pack(CloudDataSendBuf_t *buf)
{
	...
	if (zHcuSysEngPar.cloud.svrBhItfFrameStdDefault == HCU_SYSCFG_CLOUD_BH_ITF_STD_XML)
	{
		...
		if (func_cloudvela_stdxml_msg_pack(&xmlFormat, buf) == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_CLOUDVELA]++;
...
OPSTAT func_cloudvela_stdzhb_msg_unpack(msg_struct_com_cloudvela_data_rx_t *rcv)
{
	...
	if (totalLen == 0){
		//心跳接收的特殊处理
		if (flag == 1){
			if (func_cloudvela_hb_link_active_rcv_signal_check(0) == FAILURE){
				zHcuSysStaPm.taskRunErrCnt[TASK_ID_CLOUDVELA]++;
...
//For alarm report
//rcv输入参数，buf输出参数
OPSTAT func_cloudvela_stdzhb_msg_alarm_pack(UINT8 msgType, UINT8 cmdId, UINT8 useroptid, UINT8 cmdIdBackType, UINT8 alarmType, UINT32 alarmContent, UINT8 equID, UINT8 alarmSeverity, UINT8 alarmClearFlag, char *alarmDescrition, UINT32 timeStamp, CloudDataSendBuf_t *buf)
{
	//参数检查，其它参数无所谓
	if (buf == NULL){
		HcuErrorPrint("CLOUDVELA: Error CloudDataSendBuf_t pointer!\n");
	...
	if (zHcuSysEngPar.cloud.svrBhItfFrameStdDefault == HCU_SYSCFG_CLOUD_BH_ITF_STD_XML)
	{
		...
		strcpy(xmlFormat.FuncFlag, alarmDescrition);//for HKvision photo when PM25 valuse exceed the threshold
		sprintf(xmlFormat.conTimeStamp, "%08X", timeStamp);

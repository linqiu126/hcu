...
HcuFsmStateItem_t HcuFsmTimer[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_COMMON,            			fsm_timer_restart},
...
OPSTAT fsm_timer_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("TIMER: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_TIMER].taskName, zHcuVmCtrTab.task[src_id].taskName);
			return FAILURE;
		}
	}
...
OPSTAT fsm_timer_restart(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (ret == FAILURE){
		zHcuSysStaPm.taskRunErrCnt[TASK_ID_TIMER]++;
		HcuErrorPrint("TIMER: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_TIMER].taskName, zHcuVmCtrTab.task[TASK_ID_SVRCON].taskName);
		return FAILURE;
	}
	return SUCCESS;
}
		...
		for (i=0;i<MAX_TIMER_NUM_IN_ONE_HCU_1S;i++)
		{
			...
			if (zHcuTimerTable.timer1s[i].status == TIMER_STATUS_ACTIVE){
				...
				if (zHcuTimerTable.timer1s[i].elapse >= 1){
					...
					if (ret == FAILURE){
						zHcuSysStaPm.taskRunErrCnt[TASK_ID_TIMER]++;
						HcuErrorPrint("TIMER: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_TIMER].taskName, zHcuVmCtrTab.task[zHcuTimerTable.timer1s[i].taskId].taskName);
						return;
					}
	...
	for (i=0;i<MAX_TIMER_NUM_IN_ONE_HCU_10MS;i++)
	{
		...
		if (zHcuTimerTable.timer10ms[i].status == TIMER_STATUS_ACTIVE){
			...
			if (zHcuTimerTable.timer10ms[i].elapse >= 1){
				...
				if (ret == FAILURE){
					zHcuSysStaPm.taskRunErrCnt[TASK_ID_TIMER]++;
					HcuErrorPrint("TIMER: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_TIMER].taskName, zHcuVmCtrTab.task[zHcuTimerTable.timer10ms[i].taskId].taskName);
					return;
				}
			}//Elapse <= 0, timeout reach
		}
	...
	for (i=0;i<MAX_TIMER_NUM_IN_ONE_HCU_1MS;i++)
	{
		...
		if (zHcuTimerTable.timer1ms[i].status == TIMER_STATUS_ACTIVE){
			...
			if (zHcuTimerTable.timer1ms[i].elapse >= 1){
				...
				if (ret == FAILURE){
					zHcuSysStaPm.taskRunErrCnt[TASK_ID_TIMER]++;
					HcuErrorPrint("TIMER: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_TIMER].taskName, zHcuVmCtrTab.task[zHcuTimerTable.timer1ms[i].taskId].taskName);
					return;
				}
			}//Elapse <= 0, timeout reach
		}

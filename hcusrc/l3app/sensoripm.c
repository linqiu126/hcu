...
gTaskIpmContext_t gTaskIpmContext;
...
OPSTAT fsm_ipm_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("IPM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_IPM].taskName, zHcuVmCtrTab.task[src_id].taskName);
			return FAILURE;
		}
	}
...
OPSTAT fsm_ipm_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_IPM] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		//减少重复RESTART的概率
		zHcuSysStaPm.taskRunErrCnt[TASK_ID_IPM] = zHcuSysStaPm.taskRunErrCnt[TASK_ID_IPM] - HCU_RUN_ERROR_LEVEL_2_MAJOR;
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_IPM]++;
			HcuErrorPrint("IPM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_IPM].taskName, zHcuVmCtrTab.task[TASK_ID_IPM].taskName);
			return FAILURE;
		}
	}
...
OPSTAT fsm_ipm_nbiotcj188_data_req(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (ret == FAILURE){
		zHcuSysStaPm.taskRunErrCnt[TASK_ID_IPM]++;
		HcuErrorPrint("IPM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_IPM].taskName, zHcuVmCtrTab.task[TASK_ID_NBIOTCJ188].taskName);
		return FAILURE;
	}

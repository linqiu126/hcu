...
OPSTAT fsm_igm_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("IGM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_IGM].taskName, zHcuTaskInfo[src_id].taskName);
			return FAILURE;
		}
	}
...
OPSTAT fsm_igm_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuRunErrCnt[TASK_ID_IGM] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_IGM]++;
			HcuErrorPrint("IGM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_IGM].taskName, zHcuTaskInfo[TASK_ID_IGM].taskName);
			return FAILURE;
		}
	}
...
OPSTAT fsm_igm_nbiotcj188_data_req(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (ret == FAILURE){
		zHcuRunErrCnt[TASK_ID_IGM]++;
		HcuErrorPrint("IGM: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_IGM].taskName, zHcuTaskInfo[TASK_ID_NBIOTCJ188].taskName);
		return FAILURE;
	}

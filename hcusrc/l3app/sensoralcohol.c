...
OPSTAT fsm_alcohol_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_ALCOHOL, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_alcohol_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_ALCOHOL, FSM_STATE_ALCOHOL_INITED);
	...
	//启动周期性定时器
	hcu_timer_start(TASK_ID_ALCOHOL, HCU_TIMERID_WITH_DUR(TIMER_ID_1S_ALCOHOL_PERIOD_READ), TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);

	//设置状态机到目标状态
	FsmSetState(TASK_ID_ALCOHOL, FSM_STATE_ALCOHOL_ACTIVED);
	HCU_DEBUG_PRINT_FAT("ALCOHOL: Enter FSM_STATE_ALCOHOL_ACTIVED status, Keeping refresh here!\n");
	/*

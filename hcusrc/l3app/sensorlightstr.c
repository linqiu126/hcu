...
** FSM of the LIGHTSTR
*/
HcuFsmStateItem_t HcuFsmLightstr[] =
{
    ...
    {MSG_ID_COM_INIT,       	FSM_STATE_LIGHTSTR_INITED,            	fsm_lightstr_init},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_LIGHTSTR_INITED,            	fsm_com_do_nothing},
    ...
    {MSG_ID_COM_RESTART,					FSM_STATE_COMMON,            			fsm_lightstr_restart},
	{MSG_ID_COM_TIME_OUT,       			FSM_STATE_COMMON,          				fsm_lightstr_time_out},

    //Task level initialization
	{MSG_ID_CLOUDVELA_LIGHTSTR_DATA_REQ,		FSM_STATE_LIGHTSTR_ACTIVED,      	  	fsm_lightstr_cloudvela_data_req},
	{MSG_ID_CLOUDVELA_LIGHTSTR_DATA_CONFIRM,	FSM_STATE_LIGHTSTR_ACTIVED,      	  	fsm_lightstr_cloudvela_data_confirm},
	{MSG_ID_L3AQYC_EXG_CTRL_REQ,				FSM_STATE_LIGHTSTR_ACTIVED,      	  	fsm_lightstr_l3aqyc_exg_ctrl_req},
...
gTaskLightstrContext_t gTaskLightstrContext;
...
OPSTAT fsm_lightstr_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_LIGHTSTR, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_lightstr_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("LIGHTSTR: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_LIGHTSTR].taskName, zHcuVmCtrTab.task[src_id].taskName);
			return FAILURE;
		}
	}

	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_LIGHTSTR, FSM_STATE_LIGHTSTR_INITED);
	...
	memset(&gTaskLightstrContext, 0, sizeof(gTaskLightstrContext_t));

	//启动周期性定时器
	hcu_timer_start(TASK_ID_LIGHTSTR, HCU_TIMERID_WITH_DUR(TIMER_ID_1S_LIGHTSTR_PERIOD_READ), TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);

	//设置状态机到目标状态
	FsmSetState(TASK_ID_LIGHTSTR, FSM_STATE_LIGHTSTR_ACTIVED);
	HCU_DEBUG_PRINT_FAT("LIGHTSTR: Enter FSM_STATE_LIGHTSTR_ACTIVED status, Keeping refresh here!\n");
...
OPSTAT fsm_lightstr_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuSysStaPm.taskRunErrCnt[TASK_ID_LIGHTSTR] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_LIGHTSTR]++;
			HcuErrorPrint("LIGHTSTR: Send message error, TASK [%s] to TASK[%s]!\n", zHcuVmCtrTab.task[TASK_ID_LIGHTSTR].taskName, zHcuVmCtrTab.task[TASK_ID_LIGHTSTR].taskName);
			return FAILURE;
		}
	}
...
OPSTAT fsm_lightstr_cloudvela_data_req(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	return SUCCESS;
}

//收到来自CLOUD和后台云的命令，从而重新配置本地控制信息
OPSTAT fsm_lightstr_cloudvela_data_confirm(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	return SUCCESS;
}

OPSTAT fsm_lightstr_l3aqyc_exg_ctrl_req(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	return SUCCESS;
}

...
OPSTAT fsm_temp_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("TEMP: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_TEMP].taskName, zHcuTaskInfo[src_id].taskName);
			return FAILURE;
		}
	}
...
OPSTAT fsm_temp_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuRunErrCnt[TASK_ID_TEMP] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_TEMP]++;
			HcuErrorPrint("TEMP: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_TEMP].taskName, zHcuTaskInfo[TASK_ID_TEMP].taskName);
			return FAILURE;
		}
	}
...
void func_temp_time_out_read_data_from_modbus(void)
{
	...
	//如果当前传感器处于空闲态，干活！
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_TEMP]++;
			HcuErrorPrint("TEMP: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_TEMP].taskName, zHcuTaskInfo[TASK_ID_MODBUS].taskName);
			return;
		}
...
void func_temp_time_out_read_data_from_spibusaries(void)
{
	...
	//如果当前传感器处于空闲态，干活！
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_TEMP]++;
			HcuErrorPrint("TEMP: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_TEMP].taskName, zHcuTaskInfo[TASK_ID_SPIBUSARIES].taskName);
			return;
		}
...
OPSTAT fsm_temp_data_report_from_modbus(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//在线模式
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_TEMP]++;
			HcuErrorPrint("TEMP: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_TEMP].taskName, zHcuTaskInfo[TASK_ID_CLOUDVELA].taskName);
			return FAILURE;
		}

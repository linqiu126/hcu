...
OPSTAT fsm_pm25_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("PM25: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_PM25].taskName, zHcuTaskInfo[src_id].taskName);
			return FAILURE;
		}
	}
...
OPSTAT fsm_pm25_time_out(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (zHcuRunErrCnt[TASK_ID_PM25] > HCU_RUN_ERROR_LEVEL_2_MAJOR){
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_PM25]++;
			HcuErrorPrint("PM25: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_PM25].taskName, zHcuTaskInfo[TASK_ID_PM25].taskName);
			return FAILURE;
		}
	}
...
void func_pm25_time_out_read_data_from_modbus(void)
{
	...
	//如果当前传感器处于空闲态，干活！
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_PM25]++;
			HcuErrorPrint("PM25: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_PM25].taskName, zHcuTaskInfo[TASK_ID_MODBUS].taskName);
			return;
		}
...
OPSTAT fsm_pm25_data_report_from_modbus(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//在线模式
		...
		if (ret == FAILURE){
			zHcuRunErrCnt[TASK_ID_PM25]++;
			HcuErrorPrint("PM25: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskInfo[TASK_ID_PM25].taskName, zHcuTaskInfo[TASK_ID_CLOUDVELA].taskName);
			return FAILURE;
		}
...
OPSTAT func_pm25_time_out_read_data_from_bmpd300(void)
{
	...
	//存入数据库
	if ((HCU_DB_SENSOR_SAVE_FLAG == HCU_DB_SENSOR_SAVE_FLAG_YES) && (zHcuI2cPm25Bmpd300 >= HCU_SENSOR_PM25_VALUE_MIN) && (zHcuI2cPm25Bmpd300 <= HCU_SENSOR_PM25_VALUE_MAX))
	{

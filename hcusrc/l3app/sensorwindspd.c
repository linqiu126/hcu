...
HcuFsmStateItem_t HcuFsmWindspd[] =
{
    ...
    {MSG_ID_COM_INIT_FEEDBACK,			FSM_STATE_WINDSPD_ACTIVED,          fsm_com_do_nothing},
	{MSG_ID_COM_HEART_BEAT,       		FSM_STATE_WINDSPD_ACTIVED,          fsm_com_heart_beat_rcv},
	{MSG_ID_COM_HEART_BEAT_FB,       	FSM_STATE_WINDSPD_ACTIVED,          fsm_com_do_nothing},
    ...
    {MSG_ID_COM_INIT_FEEDBACK,			FSM_STATE_WINDSPD_OPT_WFFB,         fsm_com_do_nothing},
	{MSG_ID_COM_HEART_BEAT,       		FSM_STATE_WINDSPD_OPT_WFFB,         fsm_com_heart_beat_rcv},
	{MSG_ID_COM_HEART_BEAT_FB,       	FSM_STATE_WINDSPD_OPT_WFFB,         fsm_com_do_nothing},
...
OPSTAT fsm_windspd_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//启动周期性定时器
	ret = hcu_timer_start(TASK_ID_WINDSPD, TIMER_ID_1S_WINDSPD_PERIOD_READ, \
			zHcuSysEngPar.timer.array[TIMER_ID_1S_WINDSPD_PERIOD_READ].dur, TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);
	if (ret == FAILURE){
...
void func_windspd_time_out_read_data_from_modbus(void)
{
	...
	//如果当前传感器处于空闲态，干活！
		...
		//启动一次性定时器
		ret = hcu_timer_start(TASK_ID_WINDSPD, TIMER_ID_1S_WINDSPD_MODBUS_FB, \
				zHcuSysEngPar.timer.array[TIMER_ID_1S_WINDSPD_MODBUS_FB].dur, TIMER_TYPE_ONE_TIME, TIMER_RESOLUTION_1S);
		if (ret == FAILURE){
			zHcuSysStaPm.taskRunErrCnt[TASK_ID_WINDSPD]++;
			HcuErrorPrint("WINDSPD: Error start timer!\n");
			return;

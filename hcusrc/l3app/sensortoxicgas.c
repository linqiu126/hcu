...
HcuFsmStateItem_t HcuFsmToxicgas[] =
{
	...
	{MSG_ID_L3AQYC_EXG_CTRL_REQ,				FSM_STATE_TOXICGAS_ACTIVED,      	  	fsm_toxicgas_l3aqyc_exg_ctrl_req},
...
OPSTAT fsm_toxicgas_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_TOXICGAS, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_toxicgas_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_TOXICGAS, FSM_STATE_TOXICGAS_INITED);
	...
	//启动周期性定时器
	hcu_timer_start(TASK_ID_TOXICGAS, HCU_TIMERID_WITH_DUR(TIMER_ID_1S_TOXICGAS_PERIOD_READ), TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);

	//设置状态机到目标状态
	FsmSetState(TASK_ID_TOXICGAS, FSM_STATE_TOXICGAS_ACTIVED);
	HCU_DEBUG_PRINT_FAT("TOXICGAS: Enter FSM_STATE_TOXICGAS_ACTIVED status, Keeping refresh here!\n");
...
OPSTAT func_toxicgas_time_out_read_data_from_mq135(void)
{
	...
	//存入数据库
	if ((zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOTOXICGASMQ135].fVal >= HCU_SENSOR_TOXICGAS_VALUE_MIN) && (zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOTOXICGASMQ135].fVal <= HCU_SENSOR_TOXICGAS_VALUE_MAX))
	{
...
OPSTAT func_toxicgas_time_out_read_data_from_zp01voc(void)
{
	...
	//存入数据库
	if ((zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOTOXICGASZP01VOC].fVal >= HCU_SENSOR_TOXICGAS_VALUE_MIN) && (zHcuVmCtrTab.codab.si[SENSOR_ID_GPIOTOXICGASZP01VOC].fVal <= HCU_SENSOR_TOXICGAS_VALUE_MAX))
	{
...
OPSTAT fsm_toxicgas_l3aqyc_exg_ctrl_req(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	return SUCCESS;
}

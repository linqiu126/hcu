...
HcuFsmStateItem_t HcuFsmIhm[] =
{
	...
	{MSG_ID_COM_INIT,       					FSM_STATE_IHM_INITED,            		fsm_ihm_init},
	{MSG_ID_COM_INIT_FEEDBACK,					FSM_STATE_IHM_INITED,            		fsm_com_do_nothing},
    ...
    {MSG_ID_COM_RESTART,						FSM_STATE_COMMON,            			fsm_ihm_restart},
	{MSG_ID_COM_TIME_OUT,       				FSM_STATE_COMMON,          				fsm_ihm_time_out},

	//Normal working status
	{MSG_ID_NBIOTCJ188_IHM_DATA_REQ,     		FSM_STATE_IHM_ACTIVED,      			fsm_ihm_nbiotcj188_data_req},
	{MSG_ID_NBIOTCJ188_IHM_CTRL_REQ,  			FSM_STATE_IHM_ACTIVED,          		fsm_ihm_nbiotcj188_ctrl_req},
...
gTaskIhmContext_t gTaskIhmContext;
...
OPSTAT fsm_ihm_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	FsmSetState(TASK_ID_IHM, FSM_STATE_IDLE);
	return SUCCESS;
}
...
OPSTAT fsm_ihm_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	//收到初始化消息后，进入初始化状态
	FsmSetState(TASK_ID_IHM, FSM_STATE_IHM_INITED);
	...
	memset(&gTaskIhmContext, 0, sizeof(gTaskIhmContext_t));

	//启动周期性定时器
	hcu_timer_start(TASK_ID_IHM, HCU_TIMERID_WITH_DUR(TIMER_ID_1S_IHM_PERIOD_READ), TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);

	//设置状态机到目标状态
	FsmSetState(TASK_ID_IHM, FSM_STATE_IHM_ACTIVED);
	HCU_DEBUG_PRINT_FAT("IHM: Enter FSM_STATE_IHM_ACTIVED status, Keeping refresh here!\n");
	/*
...
//暂时不用，留待未来使用
OPSTAT fsm_ihm_nbiotcj188_ctrl_req(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	return SUCCESS;
}

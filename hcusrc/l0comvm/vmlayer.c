...
HcuSysEngTimerStaticCfg_t zHcuSysEngTimerStaticCfg[] = {
	...
	{TIMER_ID_1S_SYSSWM_PERIOD_WORKING,              "TID_1S_SYSSWM_PERIOD_WORKING",           90,     TIMER_RESOLUTION_1S},//900, 正常15一次，分别来轮４个软件体
	{TIMER_ID_1S_SYSSWM_SEG_DL_WAIT,             	 "TID__1S_SYSSWM_SEG_DL_WAIT",             10,      TIMER_RESOLUTION_1S},
	...
	{TIMER_ID_1S_L3BFSC_ERROR_INQ,           		 "TIMER_ID_1S_L3BFSC_ERROR_INQ",           60,      TIMER_RESOLUTION_1S},
	...
	{TIMER_ID_1S_L3BFSC_ERROR_INQ,          		 "TIMER_ID_1S_L3BFSC_ERROR_INQ",           15,      TIMER_RESOLUTION_1S},
...
UINT32 hcu_message_send(UINT32 msg_id, UINT32 dest_id, UINT32 src_id, void *param_ptr, UINT32 param_len)
{
	...
	switch (zHcuSysEngPar.traceMode)
	{
		...
		case HCU_SYSCFG_TRACE_MSG_MODE_ALL_BUT_TIME_OUT_AND_HEART_BEAT:
			...
			hcu_vm_taskid_to_string(src_id, s2);
			hcu_vm_msgid_to_string(msg_id, s3);
			if ((msg_id != MSG_ID_COM_TIME_OUT) && (msg_id != MSG_ID_COM_HEART_BEAT) && (msg_id != MSG_ID_COM_HEART_BEAT_FB)  && (msg_id != MSG_ID_USBCAN_L2FRAME_RCV)){
				HcuDebugPrint("MSGTRC: MSGID=0X%04X%s, DID=%02X%s, SID=%02X%s, LEN=%d.\n", msg_id, s3, dest_id, s1, src_id, s2, param_len);
			}
			break;

...
OPSTAT fsm_i2c_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	while(1){
		...
		if (dataLen > 1){
			//发送数据给HSMMP
			msg_struct_i2c_hsmmp_data_rx_t snd;
			memset(&snd, 0, sizeof(msg_struct_i2c_hsmmp_data_rx_t));
			snd.length = sizeof(msg_struct_i2c_hsmmp_data_rx_t);
			ret = hcu_message_send(MSG_ID_I2C_HSMMP_DATA_RX, TASK_ID_HSMMP, TASK_ID_I2C, &snd, snd.length);
			if (ret == FAILURE){
				zHcuRunErrCnt[TASK_ID_I2C]++;
				HcuErrorPrint("I2C: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_I2C], zHcuTaskNameList[TASK_ID_HSMMP]);
				return FAILURE;
			}
		}

...
FsmStateItem_t FsmI2cbuslibra[] =
{
    ...
    {MSG_ID_COM_RESTART,		FSM_STATE_IDLE,            				fsm_i2cbuslibra_restart},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_IDLE,            				fsm_com_do_nothing},

	//Task level initialization
    {MSG_ID_COM_INIT,       	FSM_STATE_I2CBUSLIBRA_INITIED,            	fsm_i2cbuslibra_init},
    {MSG_ID_COM_RESTART,		FSM_STATE_I2CBUSLIBRA_INITIED,            	fsm_i2cbuslibra_restart},
    {MSG_ID_COM_INIT_FEEDBACK,	FSM_STATE_I2CBUSLIBRA_INITIED,            	fsm_com_do_nothing},

    //Task level initialization
    {MSG_ID_COM_RESTART,        FSM_STATE_I2CBUSLIBRA_ACTIVED,            	fsm_i2cbuslibra_restart},
...
OPSTAT fsm_i2cbuslibra_task_entry(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (FsmSetState(TASK_ID_I2CBUSLIBRA, FSM_STATE_IDLE) == FAILURE){
		HcuErrorPrint("I2CBUSLIBRA: Error Set FSM State at fsm_i2cbuslibra_task_entry\n");}
	return SUCCESS;
}
...
OPSTAT fsm_i2cbuslibra_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if ((src_id > TASK_ID_MIN) &&(src_id < TASK_ID_MAX)){
		...
		if (ret == FAILURE){
			HcuErrorPrint("I2CBUSLIBRA: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_I2CBUSLIBRA], zHcuTaskNameList[src_id]);
			return FAILURE;
		}
	}

	//收到初始化消息后，进入初始化状态
	if (FsmSetState(TASK_ID_I2CBUSLIBRA, FSM_STATE_I2CBUSLIBRA_INITED) == FAILURE){
	...
	if ((zHcuSysEngPar.debugMode & TRACE_DEBUG_FAT_ON) != FALSE){
		HcuDebugPrint("I2CBUSLIBRA: Enter FSM_STATE_I2CBUSLIBRA_ACTIVED status, Keeping refresh here!\n");
	}
	/*

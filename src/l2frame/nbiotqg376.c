...
OPSTAT fsm_nbiotqg376_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	while(1){
		...
		if (dataLen > 1){
			//发送数据给HSMMP
			msg_struct_nbiotqg376_hsmmp_data_rx_t snd;
			memset(&snd, 0, sizeof(msg_struct_nbiotqg376_hsmmp_data_rx_t));
			snd.length = sizeof(msg_struct_nbiotqg376_hsmmp_data_rx_t);
			ret = hcu_message_send(MSG_ID_NBIOTQG376_HSMMP_DATA_RX, TASK_ID_HSMMP, TASK_ID_NBIOTQG376, &snd, snd.length);
			if (ret == FAILURE){
				zHcuRunErrCnt[TASK_ID_NBIOTQG376]++;
				HcuErrorPrint("NBIOTQG376: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_NBIOTQG376], zHcuTaskNameList[TASK_ID_HSMMP]);
				return FAILURE;
			}
		}

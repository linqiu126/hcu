...
FsmStateItem_t FsmPm25[] =
{
	...
	{MSG_ID_CLOUD_PM25_DATA_REQ,       	FSM_STATE_PM25_ACTIVED,          fsm_pm25_cloud_data_req},
	{MSG_ID_CLOUD_PM25_CONTROL_CMD,      FSM_STATE_PM25_ACTIVED,         fsm_pm25_cloud_control_cmd},
...
OPSTAT fsm_pm25_init(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	ret = hcu_message_send(MSG_ID_COM_INIT_FEEDBACK, TASK_ID_SVRCON, TASK_ID_PM25, &snd, snd.length);
	if (ret == FAILURE){
		HcuErrorPrint("PM25: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_PM25], zHcuTaskNameList[TASK_ID_SVRCON]);
		return FAILURE;
	}
	...
	//启动周期性定时器
	ret = hcu_timer_start(TASK_ID_PM25, TIMER_ID_1S_PM25_PERIOD_READ, PM25_TIMER_DURATION_PERIOD_READ, TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);
	if (ret == FAILURE){
...
void func_pm25_time_out_read_data_from_modbus(void)
{
	...
	//如果当前传感器处于空闲态，干活！
		...
		snd.equId = zSensorPm25Info[currentSensorPm25Id].equId;
		snd.cmdId = L3CI_pm25;
...
OPSTAT fsm_pm25_data_report_from_modbus(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	...
	if (FsmGetState(TASK_ID_CLOUD) == FSM_STATE_CLOUDCONT_OFFLINE){
		...
		if (rcv.cmdIdBackType == L3CI_cmdid_back_type_period){
			...
			record.pm1d0Value = rcv.pm1d0Value;
			record.pm2d5Value = rcv.pm2d5Value;
			record.pm10Value = rcv.pm10Value;
	...
	//在线模式
		...
		//Online processing
		//赋值给发送消息
		msg_struct_pm25_cloud_data_resp_t snd;
		memset(&snd, 0, sizeof(msg_struct_pm25_cloud_data_resp_t));
		snd.length = sizeof(msg_struct_pm25_cloud_data_resp_t);
		...
		snd.pm1d0Value = rcv.pm1d0Value;
		snd.pm2d5Value = rcv.pm2d5Value;
		snd.pm10Value = rcv.pm10Value;
		snd.gps_x = rcv.gps_x;
		snd.gps_y = rcv.gps_y;
		snd.gps_z = rcv.gps_z;
		ret = hcu_message_send(MSG_ID_PM25_CLOUD_DATA_RESP, TASK_ID_CLOUD, TASK_ID_PM25, &snd, snd.length);
		if (ret == FAILURE){
			HcuErrorPrint("PM25: Send message error, TASK [%s] to TASK[%s]!\n", zHcuTaskNameList[TASK_ID_PM25], zHcuTaskNameList[TASK_ID_CLOUD]);
			return FAILURE;
		}
	...
	//差错情形
		...
		HcuErrorPrint("PM25: Wrong state of CLOUDCONT when data need send out!\n");
...
//状态机方面，会有点交叉混淆的
//瞬时模式，有待完成，初期的必要性不大
OPSTAT fsm_pm25_cloud_data_req(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	//int ret=0;
	msg_struct_cloud_pm25_data_req_t rcv;
	memset(&rcv, 0, sizeof(msg_struct_cloud_pm25_data_req_t));
	if ((param_ptr == NULL || param_len > sizeof(msg_struct_cloud_pm25_data_req_t))){
		HcuErrorPrint("PM25: Receive message error!\n");
		return FAILURE;
	}
	memcpy(&rcv, param_ptr, param_len);
...
OPSTAT fsm_pm25_cloud_control_cmd(UINT32 dest_id, UINT32 src_id, void * param_ptr, UINT32 param_len)
{
	return SUCCESS;
}
